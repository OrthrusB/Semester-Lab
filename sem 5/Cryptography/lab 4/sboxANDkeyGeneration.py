...
# S-Box (S1 only for demonstration)
S_BOXES = [
    [
...
def feistel_function(R, round_key):
    ...
    substituted = []
    for i in range(0, len(xored), 6):  # Simulate 8 S-boxes by repeating S1
        substituted.extend([int(b) for b in sbox_substitution(xored[i:i+6], S_BOXES[0])])
    return substituted[:32]  # Ensure 32-bit output
